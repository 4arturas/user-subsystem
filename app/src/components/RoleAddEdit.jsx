...
export default function RoleAddEdit( { id, name } )
{
    ...
    const [errorRoleName, setErrorRoleName]         = React.useState('');
    React.useEffect( () => {
        if ( id )
            setRoleName( name );
    }, [] );
    const validateRoleName = ( value ) =>
    {
        if ( value.length === 0 )
        {
            setErrorRoleName( 'More than 0 symbols' );
            return false;
        }
        else if ( value.length > 11 )
        {
            setErrorRoleName( 'Less or equal 11 to symbols' );
            return false;
        }
        else
        {
    ...
    return <span>
        { id ?
            ...
            <Button variant="contained" color="primary" onClick={()=>{ setOpen( true ); }}>Add New Role</Button>
        }
                        ...
                        <tr>
                            <td>
                                <TextField label='Role Name'
                                           helperText={ errorRoleName }
                                           error={ errorRoleName.length > 0 }
                                           onChange={ (event) => {
                        ...
                        <tr>
                            <td>
                                <Button ref={refButton} variant="contained" color="primary" onClick={ async () => {
                                    const valid = validateRoleName( roleName );
                                    if ( !valid )
                                    {
                                        ReactDOM.render(
                                    ...
                                    let jSonResponse;
                                    if ( id )
                                        jSonResponse = await API.update_Role( id, roleName );
                                    else
                                        jSonResponse = await API.add_Role( roleName );
                                    ...
                                    ReactDOM.render(
                                        <Alert severity="success">Role was successfully {id?'updated':'added'}</Alert>
                                        ,refMsg.current );

                                }}>{ id ? 'Edit Role' : 'Add Role' }</Button>
                                &nbsp;&nbsp;&nbsp;
                                <Button ref={refCancel} variant="contained" color="primary" onClick={ () => { setOpen(false ); } }>Cancel</Button>
                                <Button ref={refClose} style={{display:'none'}} variant="contained" color="primary" onClick={ () => { window.location = id ? `/roles/role?id=${id}` : `/roles` } }>Close</Button>
                                <div ref={refMsg}></div>
                            </td>
                        </tr>
                    </tbody>
