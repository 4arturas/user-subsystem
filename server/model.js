...
async function get_RoleGroupByName( roleGroupName )
{
    const res = await pool.query(`SELECT ROW_TO_JSON(rg) FROM role_groups as rg where rg.role_group_name='${roleGroupName}'`);
    console.log( 'rows: ' + res.rows.length );
    if ( res.rows.length === 0 )
        return null;
    return res.rows[0].row_to_json;
...
async function update_RoleGroup( roleGroupId, roleGroupName )
{
    const res = await pool.query( `update role_groups set role_group_name='${roleGroupName}' where role_group_id=${roleGroupId}` );
    return { ok: 1 };
}
