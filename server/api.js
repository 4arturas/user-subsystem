...
app.get('/api/users/rolegroups/belong/info', async (req, res) =>
{
    const userId = req.query.userId;
    const jSonRoles = await model.get_RoleGroupsWithBelongToUserInfo( userId );
...
app.post('/api/rolegroups/update', async (req, res) =>
{
    const jSon = req.body;
    const roleGroupName = jSon.roleGroupName;
    const roleGroup = await model.get_RoleGroupByName( roleGroupName );
    if ( roleGroup !== null )
    {
        return res.json( { error: 'Role Group exists by given name' } );
    }
    const roleGroupId= jSon.roleGroupId;
    const jSonAddResult = await model.update_RoleGroup( roleGroupId, roleGroupName );
    res.json( jSonAddResult );
});
...
app.post('/api/roles/rolegroups/detach', async (req, res) =>
{
    const jSon = req.body;
    const roleId = jSon.roleId;
    const roleGroupId = jSon.roleGroupId;
    const jSonAddResult = await model.detach_RoleFromRoleGroups( roleId, roleGroupId );
    res.json( jSonAddResult );
});
